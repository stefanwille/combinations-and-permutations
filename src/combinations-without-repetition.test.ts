const nChooseK = <T extends number>(elements: T[], k: number): T[][] => {
	...
	const combinationsWithFirstElement: Combination[] = combinationsOfSubElements.map((combination) => [
		firstElement,
		...combination,
	]);
	const combinationsWithoutFirstElement = nChooseK(remainingElements, k);
	const result: Combination[] = combinationsWithFirstElement.concat(combinationsWithoutFirstElement);
	return result;
};

describe('nChooseK()', () => {
	it('returns the combinations without repetition for the given set', () => {
		expect(nChooseK<number>([ 6, 7, 8 ], 2)).toEqual([ [ 6, 7 ], [ 6, 8 ], [ 7, 8 ] ]);
	});

	it('handles k=3', () => {
		expect(nChooseK<number>([ 5, 6, 7, 8 ], 3)).toEqual([ [ 5, 6, 7 ], [ 5, 6, 8 ], [ 5, 7, 8 ], [ 6, 7, 8 ] ]);
	});
